...
/// @title MockArcStablecoin
/// @notice The one local stand-in for an Arc EIP-3009 stablecoin. USDC and EURC are
///         this contract with a different constructor argument.
///
...
///      **no fork test can complete a transfer** (finding 3). Every invariant, every
///      waterfall assertion and every keeper-market property is therefore checked
///      against this contract. If it lies, the whole local suite lies with it — which
///      is why the deviations from the real token below are enumerated rather than
///      left to be discovered.
///
///      **One implementation, two currencies, and that is a correctness property
///      rather than a tidiness one.** Phase 7 needs a EURC mock. Copying this file
///      would create two EIP-3009 implementations of which only one is exercised
///      hard, and the unexercised one drifts — the drift showing up much later as a
///      signature that verifies in test and is rejected on chain, which is the most
///      expensive shape a test-only bug can take. So `name` and `symbol` are
///      constructor arguments and everything else is shared. In particular the three
///      typehashes exist in this file and nowhere else in the mock tree.
///
///      **Two currencies, one shape, measured rather than assumed.** Plan 07-01 read
///      EURC (`0x89B50855…`) live on chain 5042002 and recorded finding 31:
///      `decimals() == 6`, `version() == "2"`, and all three typehashes byte-identical
///      to USDC's, with a domain separator that matched the four-field derivation
///      exactly. So `version` and `decimals` stay `constant` here — they are measured
///      facts about *both* tokens, not defaults — and only the two strings vary.
///
...
///      - A domain separator built from `(name, "2", chainId, address(this))` — note
///        `"USDC"`, not `"USD Coin"` — and derived rather than stored, exactly as the
///        real token does it.
...
contract MockArcStablecoin is IERC20 {
    /// @notice The EIP-712 domain's `name`, and the only field that distinguishes one
    ///         currency's separator from another's beyond `verifyingContract`.
    string public name;
